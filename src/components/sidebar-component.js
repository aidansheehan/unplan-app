...
import { useEffect, useState } from "react";
  ...
  const SidebarComponent = ({navItems, isNavOpen}) => {
    ...
    return (
<aside className={`bg-gray-200 md:bg-blue-600 flex mt-[var(--header-height)] mb-[var(--footer-height)] flex-col justify-between fixed inset-x-0 md:inset-y-0 h-[calc(100%-var(--header-height)-var(--footer-height))] md:h-auto w-full md:w-[var(--sidebar-width)] z-10 ${isNavOpen ? 'flex' : 'hidden'} md:flex`}>
