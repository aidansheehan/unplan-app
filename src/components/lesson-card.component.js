...
const LessonCard = ({ lesson }) => {
    return (
        <Link href={`/view-lesson/${lesson.id}`} className="block bg-white rounded-lg shadow-lg hover:shadow-2xl transition-shadow duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-105 cursor-pointer break-words h-fit">
                <div className="p-6 border-l-4 border-green-500 h-full"> {/* Choose between green or orange */}
                    ...
                    {
                        lesson.isOnline || lesson.isOneToOne ? (
                            <div className="mt-4 w-full" >
                                {lesson.isOnline && <p className="text-gray-800 text-sm text-right font-semibold">Online Class</p>}
                                {lesson.isOneToOne && <p className="text-gray-800 text-sm text-right font-semibold">1-to-1 Class</p>}
                            </div>
                        ) : (<></>)
                    }
